...
namespace InventoryManagementSystem
{
    public partial class CategoryForm : Form
    {
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Kamu yakin ingin menghapus kategori ini?", "Hapus Kategori", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbCategory WHERE catid LIKE '" + dgvCategory.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Kategori telah berhasil dihapus!");
                }
            }
            LoadCategory();
        }
