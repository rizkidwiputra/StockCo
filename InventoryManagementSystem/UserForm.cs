...
namespace InventoryManagementSystem
{
    public partial class UserForm : Form
    {
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Kamu yakin ingin menghapus pengguna ini?","Hapus Pengguna",MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbUser WHERE username LIKE '" + dgvUser.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("Pengguna telah berhasil dihapus!");
                }
            }
            LoadUser();
        }
